...
const CYCLE_DURATION_IN_FRAMES = CYCLE_DURATION_IN_SECONDS * FRAMES_PER_SECOND;
  ...
  sparkles.forEach(set => {
    set.forEach(
      ({
        ...
        const progress = clip(offsetCurrentFrame / animationDurationInFrames);
        const opacityProgress = clip(
          offsetCurrentFrame / (CYCLE_DURATION_IN_FRAMES * 0.05)
        );

        const opacity = transition(0, .7, opacityProgress);
